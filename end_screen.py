...
def end_screen(screen, win, score, seconds_first, seconds_second, minutes_first, minutes_second, clock):
    ...
    while True:
        ...
        if win:
            screen.fill(white)
            disp_text(screen, "How Freedom Feels?", (scr_width / 2, scr_height / 4), end_title_text_win,
                      peace_green)
            ...
            pygame.draw.rect(screen, black, (scr_width / 2 - 242, scr_height / 2 + 48, 484, 174), 2)

        # if player caught
        else:
            screen.fill(black)
            draw_walls(screen, wall_brick_width, wall_brick_height)
            disp_text(screen, "Dragged Behind Bars!!", (scr_width / 2, scr_height / 4), end_title_text_lose, blood_red)
            ...
            # displaying menu options
            if option == 0:
                disp_text(screen, "Pull It Again?", (scr_width / 2, scr_height / 2 + 80), menu_item_text_selected, silver)
            else:
                disp_text(screen, "Pull It Again?", (scr_width / 2, scr_height / 2 + 80), menu_item_text, grey)

            if option == 1:
                disp_text(screen, "Change Disguise?", (scr_width / 2, scr_height / 2 + 130), menu_item_text_selected, silver)
            else:
                disp_text(screen, "Change Disguise?", (scr_width / 2, scr_height / 2 + 130), menu_item_text, grey)

            if option == 2:
                disp_text(screen, "Give Up?", (scr_width / 2, scr_height / 2 + 180), menu_item_text_selected, silver)
            else:
                disp_text(screen, "Give Up?", (scr_width / 2, scr_height / 2 + 180), menu_item_text, grey)

            # drawing box around options
            pygame.draw.rect(screen, white, (scr_width / 2 - 250, scr_height / 2 + 40, 500, 190), 3)
            pygame.draw.rect(screen, white, (scr_width / 2 - 242, scr_height / 2 + 48, 484, 174), 2)

        # display score
        disp_text(screen, "score : ", (scr_width / 4 - 65, scr_height / 8 - 30), end_screen_text, grey)
        disp_text(screen, str(score), (scr_width / 4, scr_height / 8 + 2 - 30), end_screen_number, light_green)

        # display time
        disp_text(screen, "pursuit : ", (3 * scr_width / 4, scr_height / 8 - 30), end_screen_text, grey)
